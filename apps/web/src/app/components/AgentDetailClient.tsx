...
export function AgentDetailClient({ agent: initialAgent, initialActions }: Props) {
  ...
  return (
    <div className="px-4 sm:px-6 py-4 sm:py-6 max-w-4xl mx-auto space-y-4 sm:space-y-5 fade-in">
      {/* Breadcrumb */}
      ...
      {/* Agent header */}
      <div className="bg-surface-2 border border-border rounded-xl p-4 sm:p-6">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <h1 className="font-display text-xl sm:text-2xl font-bold text-white">{agent.name}</h1>
            <a
              href={explorerUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="text-[10px] sm:text-xs font-mono text-accent-cyan hover:text-cyan-300 mt-1 inline-flex items-center gap-1 break-all"
            >
              <span className="hidden sm:inline">{agent.publicKey}</span>
              <span className="sm:hidden">{agent.publicKey.slice(0, 16)}…{agent.publicKey.slice(-8)}</span>
              ↗
            </a>
          </div>
          <div className="flex gap-2 flex-shrink-0">
            <button
              ...
              aria-label="Refresh"
              className="min-h-[44px] px-3 py-2 text-sm font-mono border border-border rounded-lg text-slate-400 hover:text-white transition-colors"
            >
              ↻
            </button>
            <button
              ...
              disabled={loading}
              className="min-h-[44px] px-3 sm:px-4 py-2 text-xs sm:text-sm font-mono bg-accent-purple/20 border border-accent-purple/40 text-accent-purple hover:bg-accent-purple/30 rounded-lg transition-all disabled:opacity-40"
            >
              {loading ? '...' : '▶ Run Once'}
            </button>
          </div>
        ...
        {/* Balances — 1 col mobile, 3 col desktop */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4 sm:mt-5">
          <div className="bg-surface-3 border border-border rounded-lg p-3 sm:p-4">
            <p className="text-[10px] text-slate-500 tracking-widest font-mono mb-1">SOL BALANCE</p>
            <p className="text-xl sm:text-2xl font-display font-bold text-accent-cyan">{agent.solBalance.toFixed(6)}</p>
          ...
          <div className="bg-surface-3 border border-border rounded-lg p-3 sm:p-4">
            ...
            <p className="text-xl sm:text-2xl font-display font-bold text-accent-purple">{agent.splBalance.toFixed(2)}</p>
          ...
          <div className="bg-surface-3 border border-border rounded-lg p-3 sm:p-4">
            ...
            <p suppressHydrationWarning className="text-xs text-slate-600 font-mono mt-1">
              {agent.lastActionAt
                ? new Date(agent.lastActionAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })
                : ''}
            </p>
          </div>
        </div>
      ...
      <div className="bg-surface-2 border border-border rounded-xl overflow-hidden">
        ...
        {actions.length === 0 ? (
          <p className="px-5 py-10 text-center text-slate-500 font-mono text-sm">
            No actions yet. Click &ldquo;Run Once&rdquo; to execute an action.
          </p>
        ) : (
          <div className="divide-y divide-border/40">
            {actions.map((action) => (
              <button
                ...
                className="w-full flex items-center justify-between px-4 sm:px-5 py-3 sm:py-3.5 hover:bg-surface-3/40 transition-colors text-left min-h-[52px]"
              >
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    {action.signature && (
                      <p className="text-[10px] font-mono text-slate-500 mt-0.5 truncate max-w-[160px] sm:max-w-none">
                        {action.signature.slice(0, 20)}…
                      </p>
                    )}
                    {action.error && (
                      <p className="text-[10px] font-mono text-accent-red/70 mt-0.5 truncate max-w-[160px] sm:max-w-none">
                ...
                <div className="text-right flex-shrink-0 ml-2">
                  <p suppressHydrationWarning className="text-[10px] font-mono text-slate-500">
